...
window.NFTApp = window.NFTApp || {};
  ...
  updateRulesSectionVisibility: function (projectData) {
    ...
    console.log("[DEBUG] Trait layers count:", traitLayersCount, "Has minimum layers:", hasMinimumLayers)
  ...
  showCombinationRuleModal: function (projectData, ruleToEdit = null) {
    ...
    // Create modal content with wider width
    modalOverlay.innerHTML = `
  <div class="modal combination-rule-modal larger-rule-modal" style="width: 90%; max-width: 900px;">
    <button class="modal-close combination-rule-close-btn" id="close-combination-rule-modal" title="Close modal">&times;</button>
        ...
        setTimeout(() => {
          ...
          const markTraitCheckboxes = (prefix, traitObjs) => {
            ...
            const retryMarkTraitCheckboxes = (attempts = 0) => {
              ...
              if (!allFound && attempts < 5) {
                setTimeout(() => retryMarkTraitCheckboxes(attempts + 1), 50);
              }
            };
          ...
          if (ruleToEdit.traits && (!ruleToEdit.firstTraits && !ruleToEdit.secondTraits)) {
            if (ruleToEdit.appliesTo === 'traits-to-layer') markTraitCheckboxes('first', ruleToEdit.traits);
            if (ruleToEdit.appliesTo === 'layer-to-traits') markTraitCheckboxes('second', ruleToEdit.traits);
          }
        }, 200);
      }
      // --- END PATCH ---
          ...
          if (traitsList) {
            traitsList.addEventListener('click', function(e) {
              ...
              if (label) {
                ...
                label.setAttribute('data-selected', label.classList.contains('selected') ? '1' : '0');
  ...
  showSameLayerWarning: function (modalOverlay) {
    ...
    warningDiv.style.fontSize = "0.85rem" // Make text smaller
    warningDiv.style.color = "#d32f2f" // Make text red
    warningDiv.style.padding = "8px 12px"
    warningDiv.style.marginBottom = "10px"
    warningDiv.style.backgroundColor = "rgba(211, 47, 47, 0.1)"
    warningDiv.style.borderRadius = "4px"
    warningDiv.style.borderLeft = "4px solid #d32f2f"
    ...
    if (ruleSelectors) {
      ruleSelectors.parentNode.insertBefore(warningDiv, ruleSelectors)
    }
    ...
    if (saveBtn) {
      ...
      saveBtn.title = "Cannot create rules with the same layer selected in both dropdowns"
    }
  },
  ...
  updateSelectors: function (projectData, ruleToEdit = null) {
    ...
    // Reset visibility
    firstLayerSelect.style.display = "block"
    secondLayerSelect.style.display = "block"
    firstTraitsContainer.style.display = "none"
    secondTraitsContainer.style.display = "none"

    // Show/hide trait selection based on rule type and applies to
    // For stacking rules (always-above, always-below, etc.), always show trait lists
    const isStackingRule = ruleType === "always-above" || ruleType === "always-below" || ruleType === "immediately-above" || ruleType === "immediately-below";
    
    if (isStackingRule) {
      // Stacking rules: show traits based on appliesTo
      if (ruleAppliesTo === "between-layers" || ruleAppliesTo === "between-traits") {
        firstTraitsContainer.style.display = "block"
        secondTraitsContainer.style.display = "block"
      ...
      // Non-stacking rules: only show traits for specific cases
      if (ruleAppliesTo === "between-traits") {
        firstTraitsContainer.style.display = "block"
        secondTraitsContainer.style.display = "block"
      } else if (ruleAppliesTo === "layer-to-traits") {
        firstTraitsContainer.style.display = "none"
      ...
      // between-layers: keep traits hidden (default)
    }
    ...
    if (ruleType === "conditional-restriction") {
      switch (ruleAppliesTo) {
        ...
        case "between-traits":
          firstSelectorLabel.textContent = "IF Conditional Trait(s):"
          secondSelectorLabel.textContent = "THEN Allow ONLY Target Trait(s):"
          break
  ...
  updateRule: function (ruleId, projectData, modalOverlay) {
    ...
    switch (ruleAppliesTo) {
      case "between-layers": {
        // Collect selected traits from both layers (now allows trait selection)
  ...
  updateRulesUI: function (projectData) {
      ...
      let ruleHeader = `
        <div class="rule-header">
          <div class="rule-type ${ruleTypeClass}">
            ...
            <span class="rule-applies-to-text"> - ${appliesToText}</span>
          </div>
    ...
    setTimeout(() => {
      rulesList.querySelectorAll('.rule-type-text-colored').forEach(span => {
        ...
        if (classList.includes('rule-type-above')) color = '#9000ff';
        if (classList.includes('rule-type-below')) color = '#ffe400';
      ...
      rulesList.querySelectorAll('.rule-type-above-icon').forEach(svg => svg.style.setProperty('stroke', '#9000ff', 'important'));
      rulesList.querySelectorAll('.rule-type-below-icon').forEach(svg => svg.style.setProperty('stroke', '#ffe400', 'important'));
    }, 0);
