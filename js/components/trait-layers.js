...
window.NFTApp.registerModule("traitLayers", {
    ...
    if (deleteAllLayersBtn) {
      deleteAllLayersBtn.addEventListener("click", () => {
        NFTApp.getModule("confirmationModal").show(
          ...
          "Are you sure you want to delete all trait layers?",
          "This action cannot be undone and will remove all trait layers and their traits.",
          () => {
    ...
    sortedTraits.forEach((layer, index) => {
      ...
      <div class="trait-layer-header">
        ...
        <div class="trait-layer-header-rarity">
          ...
          <input type="text" class="trait-layer-header-rarity-value ${layerRarityColorClass}" value="${layer.rarity || 0}%">
        </div>
      ...
      <div class="trait-layer-content ${wasExpanded ? "expanded" : ""}">
        ...
        <div class="trait-rarity-actions">
          <button class="app-action-btn normalize-rarities-btn tooltip">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="14" height="14">
              <line x1="5" y1="12" x2="19" y2="12"></line>
              <polyline points="15 8 19 12 15 16"></polyline>
            </svg>
            Normalize Rarities
            <span class="tooltiptext">Distribute rarity equally among all traits</span>
          </button>
  ...
  // Generate trait items HTML with rarity indicators
  generateTraitItems: function (layer) {
    ...
    return sortedTraits
      .map(trait => `
        <div class="trait-item" data-id="${trait.id}">
          <div class="trait-image">
            ${
              trait.imageData
                ? `<img src="${trait.imageData}" alt="${trait.name}">`
          ...
          <span class="custom-tooltip-anchor" style="display:flex;justify-content:center;align-items:center;width:100%;margin-top:12px;">
            <span class="trait-name-ellipsis-list">${trait.name}</span>
            <span class="custom-tooltip">${trait.name}</span>
          </span>
          <!-- Removed trait-layer-name-inside-card -->
          <div class="trait-details">
            <div class="trait-rarity">
              <div class="trait-rarity-header">
                <label>Rarity:</label>
                <input type="text" class="trait-rarity-input" value="${trait.rarity.toFixed(2)}%" data-original-value="${trait.rarity}">
              </div>
              <input type="range" class="trait-rarity-slider" value="${trait.rarity}" min="0" max="${layer.rarity || 100}" step="0.5" data-original-value="${trait.rarity}">
            </div>
          </div>
          <div class="trait-actions">
            <button class="action-btn rename-trait tooltip">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
              </svg>
              <span class="tooltiptext">Rename trait</span>
            </button>
            <button class="action-btn delete-trait tooltip">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <polyline points="3 6 5 6 21 6"></polyline>
                <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path>
              </svg>
            ...
            <button class="action-btn replace-trait-image tooltip" data-trait-id="${trait.id}" data-layer-id="${layer.id}">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M12 20h9"></path>
                <path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4 12.5-12.5z"></path>
              </svg>
              <span class="tooltiptext">Replace image</span>
            </button>
            <input type="file" accept="image/*" class="replace-image-input" data-trait-id="${trait.id}" data-layer-id="${layer.id}" style="display:none">
          </div>
        </div>
      `)
      .join("");
  ...
  setupTraitLayerBarEvents: function (traitLayerBar, layer, projectData) {
    ...
    if (headerRaritySlider && headerRarityValue) {
      ...
      headerRaritySlider.addEventListener("input", (e) => {
        ...
        headerRarityValue.value = `${value}%`

        // Update color class based on new rarity value
        const colorClass = this.getRarityColorClass(Number.parseFloat(value), false, 100)
    ...
    // Normalize rarities button
    const normalizeRaritiesBtn = traitLayerBar.querySelector(".normalize-rarities-btn")
    if (normalizeRaritiesBtn) {
      normalizeRaritiesBtn.addEventListener("click", (e) => {
        e.stopPropagation()
        this.normalizeTraitRarities(layer.id, projectData)
      })
    }

    // Randomize rarities (tiered) button
    ...
    if (randomizeRaritiesTieredBtn) {
      randomizeRaritiesTieredBtn.addEventListener("click", (e) => {
        e.stopPropagation()
        this.randomizeTraitRarities(layer.id, projectData)
      })
    }

    // Randomize unique rarities button
    ...
    if (randomizeUniqueRaritiesBtn) {
      randomizeUniqueRaritiesBtn.addEventListener("click", (e) => {
        e.stopPropagation()
        this.randomizeUniqueTraitRarities(layer.id, projectData)
      })
    }

    // Rename trait buttons
    const renameTraitBtns = traitLayerBar.querySelectorAll(".rename-trait")
    renameTraitBtns.forEach((btn) => {
      btn.addEventListener("click", (e) => {
        e.stopPropagation()
        const traitItem = btn.closest(".trait-item")
        ...
        // Show rename modal for trait
        this.showRenameModal("Rename Trait", trait.name, (newName) => {
          if (!newName || newName.trim() === "") {
            this.showFeedback("Trait name cannot be empty", "error")
            return
          }

          // Check if a trait with this name already exists in the layer
          if (layer.traits.some((t) => t.id !== traitId && t.name === newName)) {
            this.showFeedback(`A trait named "${newName}" already exists in this layer`, "error")
            return
          }
          ...
          this.updateCombinationRulesAfterTraitRename(projectData, layer.id, layer.name, traitId, oldName, newName)
          ...
          this.showFeedback(`Trait renamed to "${newName}"`, "success")
        })
      })
    })

    // Delete trait buttons
    const deleteTraitBtns = traitLayerBar.querySelectorAll(".delete-trait")
    ...
    // Trait rarity slider and input field change events
    const traitRaritySliders = traitLayerBar.querySelectorAll(".trait-rarity-slider")

    traitRaritySliders.forEach((slider) => {
      const traitItem = slider.closest(".trait-item")
      const traitId = traitItem.dataset.id
      ...
      slider.addEventListener("input", (e) => {
        e.stopPropagation()
        const value = Number.parseFloat(e.target.value).toFixed(2)
        rarityInput.value = `${value}%`

        // Calculate total rarity for the layer
        const totalLayerRarity = layer.traits.reduce(
          (sum, t) => sum + (t.id === traitId ? Number.parseFloat(value) : t.rarity),
          0,
        )

        // Update color class based on new rarity value relative to total
        const colorClass = this.getRarityColorClass(Number.parseFloat(value), false, totalLayerRarity)
        rarityInput.className = `trait-rarity-input ${colorClass}`

        // Update the rarity indicator color and text
        const rarityBox = traitItem.querySelector(".trait-rarity-box")
        const rarityText = traitItem.querySelector(".trait-rarity-text")

        if (rarityBox && rarityText) {
          const bgColorClass = this.getRarityColorClass(Number.parseFloat(value), true, totalLayerRarity)
          // Remove all existing bg-rarity classes
          rarityBox.classList.forEach((cls) => {
            if (cls.startsWith("bg-rarity-")) {
              rarityBox.classList.remove(cls)
            }
          })
          // Add the new bg-rarity class
          rarityBox.classList.add(bgColorClass)

          // Update the rarity text
          rarityText.textContent = this.getRarityText(Number.parseFloat(value), totalLayerRarity)
        }
      })
    ...
    traitRarityInputFields.forEach((input) => {
      const traitItem = input.closest(".trait-item")
      const traitId = traitItem.dataset.id
      const slider = traitItem.querySelector(".trait-rarity-slider")
    ...
    // Add event listener for replace image button
    const replaceImageBtns = traitLayerBar.querySelectorAll(".replace-trait-image")
    replaceImageBtns.forEach((btn) => {
      btn.addEventListener("click", (e) => {
        e.stopPropagation()
        const traitId = btn.getAttribute("data-trait-id")
        const layerId = btn.getAttribute("data-layer-id")
        const fileInput = traitLayerBar.querySelector(`input.replace-image-input[data-trait-id='${traitId}'][data-layer-id='${layerId}']`)
  ...
  setupTraitLayerDragAndDrop: function (container, projectData) {
    ...
    container.addEventListener("drop", (e) => {
      ...
      // CRITICAL FIX: Re-setup drag and drop event listeners after reordering
      // This ensures the layers can still be expanded/collapsed after drag-and-drop
      setTimeout(() => {
        this.setupTraitLayerDragAndDrop(container, projectData);
      }, 100);
  ...
  randomizeTraitRarities: function (layerId, projectData) {
    // Find the layer
    const layer = projectData.traits.find((l) => l.id === layerId)
    if (!layer || !layer.traits || layer.traits.length === 0) return
    ...
    // Define rarity tiers
    const rarityTiers = [
      { name: "Common", chance: 0.35, rarity: 35 },
      { name: "Uncommon", chance: 0.25, rarity: 25 },
      { name: "Rare", chance: 0.2, rarity: 20 },
      { name: "Epic", chance: 0.12, rarity: 12 },
      { name: "Legendary", chance: 0.05, rarity: 5 },
      { name: "Mythic", chance: 0.03, rarity: 3 },
    ]

    // Shuffle the traits to randomize which ones get which rarity
    const shuffledTraits = [...layer.traits].sort(() => Math.random() - 0.5)

    // Assign rarities based on tiers
    const remainingTraits = shuffledTraits.length
    let currentTierIndex = 0
    let assignedInCurrentTier = 0

    shuffledTraits.forEach((trait, index) => {
      // Calculate how many traits should be in the current tier
      const currentTier = rarityTiers[currentTierIndex]
      const traitsInTier = Math.round(shuffledTraits.length * currentTier.chance)

      // Assign the rarity from the current tier
      trait.rarity = currentTier.rarity

      // Move to the next tier if we've assigned enough traits in this one
      assignedInCurrentTier++
      if (assignedInCurrentTier >= traitsInTier && currentTierIndex < rarityTiers.length - 1) {
        currentTierIndex++
        assignedInCurrentTier = 0
      }
    })

    // Normalize to ensure the total rarity equals the layer rarity
    const totalRarity = shuffledTraits.reduce((sum, trait) => sum + trait.rarity, 0)
    const normalizationFactor = layer.rarity / totalRarity

    shuffledTraits.forEach((trait) => {
      trait.rarity = trait.rarity * normalizationFactor
    })

    // Update the original traits array
    layer.traits.forEach((trait, index) => {
      const shuffledTrait = shuffledTraits.find((t) => t.id === trait.id)
      if (shuffledTrait) {
        trait.rarity = shuffledTrait.rarity
      }
    })
    ...
    this.showFeedback(`Randomized rarities for "${layer.name}" based on tiers`, "success")
  },

  // Randomize unique trait rarities - each trait gets a unique random rarity
  randomizeUniqueTraitRarities: function (layerId, projectData) {
    // Find the layer
    const layer = projectData.traits.find((l) => l.id === layerId)
    if (!layer || !layer.traits || layer.traits.length === 0) return

    // Generate unique random rarities
    ...
    for (let i = 0; i < traitCount; i++) {
      let randomValue
      do {
        randomValue = Math.random() * 0.9 + 0.1 // Random value between 0.1 and 1.0
      } while (uniqueRarities.includes(randomValue)) // Ensure uniqueness
      uniqueRarities.push(randomValue)
    }
    
    // Sort the rarities to ensure they're distributed nicely
    uniqueRarities.sort((a, b) => a - b)
    
    // Shuffle the traits to randomize which trait gets which rarity
    const shuffledTraits = [...layer.traits].sort(() => Math.random() - 0.5)
    
    // Assign the unique rarities
    shuffledTraits.forEach((trait, index) => {
      trait.rarity = uniqueRarities[index] * 100 // Convert to percentage
    })
    ...
    this.showFeedback(`Randomized unique rarities for "${layer.name}"`, "success")
  },
  ...
  updateLayerRarity: function (layerId, newRarity, projectData, preserveExpanded = true) {
    ...
    if (window.NFTApp && window.NFTApp.getModule('projectService')) {
      window.NFTApp.getModule('projectService').saveProjectData()
    }
