...
window.NFTApp.registerModule("generateNfts", {
  ...
  _initToggleListeners: function() {
    ...
    if (singleSeedToggle && singleSeedInput && generateSeedBtn) {
      ...
      singleSeedToggle.addEventListener('change', (e) => {
        ...
        // The toggle only controls whether the input is enabled, not the button itself
        // Check minimum requirements before disabling the button
        ...
        const hasMinimumRequirements = generateNftsUI && generateNftsUI._hasMinimumRequirements 
          ? generateNftsUI._hasMinimumRequirements(projectData) 
          : false;
        
        // Only disable Generate Seed button if toggle is off AND requirements are met
        // If toggle is on, button should be enabled (user can generate)
        // If toggle is off but requirements met, button should still be enabled (user can generate after toggling)
        if (!isActive) {
          // Toggle is off - disable button unless requirements aren't met (then it's already disabled)
          // Actually, if toggle is off, we should still allow the button to work if requirements are met
          // The button click handler will check for seed input anyway
          generateSeedBtn.disabled = !hasMinimumRequirements;
        } else {
          // Toggle is on - enable button if requirements are met
          generateSeedBtn.disabled = !hasMinimumRequirements;
        }
  ...
  syncCardHeights: function() {
    ...
    if (leftSideColumn && traitsCard) {
      // Don't dynamically set height - use fixed CSS height instead to prevent layout shifts
      // Fixed height is set in CSS: 676px for traits-list-card
      traitsCard.style.visibility = 'visible';
      traitsCard.style.opacity = '1';
      traitsCard.style.overflow = 'hidden';
      // Ensure fixed height is maintained
      traitsCard.style.height = '676px';
      traitsCard.style.minHeight = '676px';
      traitsCard.style.maxHeight = '676px';
      traitsCard.style.flex = 'none';
    }
  ...
  generateSingleNFT: function(projectData, overrideRules = false, seed = null, darkModeEnabled = false) {
    ...
    // CRITICAL: Start first task in order - mark selecting-traits as in-progress
    if (window.NFTApp && window.NFTApp.getModule && window.NFTApp.getModule('generateNftsUI')) {
      const generateNftsUI = window.NFTApp.getModule('generateNftsUI');
      if (generateNftsUI.updateTaskStatus) {
        generateNftsUI.updateTaskStatus('selecting-traits', 'in-progress');
      }
    }
    ...
    if (seed && typeof seed === 'string' && /^\d+$/.test(seed) && seed.length === numLayers * 2) {
      ...
      // Update task: Traits selected
      if (window.NFTApp && window.NFTApp.getModule && window.NFTApp.getModule('generateNftsUI')) {
        const generateNftsUI = window.NFTApp.getModule('generateNftsUI');
        if (generateNftsUI.updateTaskStatus) {
          generateNftsUI.updateTaskStatus('selecting-traits', 'completed');
          generateNftsUI.updateTaskStatus('loading-images', 'in-progress');
        }
      }
      ...
      return Promise.all(imagePromises).then(() => {
        // Update task: Images loaded
        if (window.NFTApp && window.NFTApp.getModule && window.NFTApp.getModule('generateNftsUI')) {
          const generateNftsUI = window.NFTApp.getModule('generateNftsUI');
          if (generateNftsUI.updateTaskStatus) {
            generateNftsUI.updateTaskStatus('loading-images', 'completed');
            generateNftsUI.updateTaskStatus('validating-rules', 'in-progress');
          }
        }
        ...
        // Update task: Rules validated
        if (window.NFTApp && window.NFTApp.getModule && window.NFTApp.getModule('generateNftsUI')) {
          const generateNftsUI = window.NFTApp.getModule('generateNftsUI');
          if (generateNftsUI.updateTaskStatus) {
            generateNftsUI.updateTaskStatus('validating-rules', 'completed');
            generateNftsUI.updateTaskStatus('composing-canvas', 'in-progress');
          }
        }
        ...
        // Update task: Canvas composed, generating image
        if (window.NFTApp && window.NFTApp.getModule && window.NFTApp.getModule('generateNftsUI')) {
          const generateNftsUI = window.NFTApp.getModule('generateNftsUI');
          if (generateNftsUI.updateTaskStatus) {
            generateNftsUI.updateTaskStatus('composing-canvas', 'completed');
            generateNftsUI.updateTaskStatus('generating-image', 'in-progress');
          }
        }
        
        const imageData = canvas.toDataURL();
        
        // Update task: Image generated
        if (window.NFTApp && window.NFTApp.getModule && window.NFTApp.getModule('generateNftsUI')) {
          const generateNftsUI = window.NFTApp.getModule('generateNftsUI');
          if (generateNftsUI.updateTaskStatus) {
            generateNftsUI.updateTaskStatus('generating-image', 'completed');
          }
        }
    ...
    // Update task: Traits selected, start loading images
    if (window.NFTApp && window.NFTApp.getModule && window.NFTApp.getModule('generateNftsUI')) {
      const generateNftsUI = window.NFTApp.getModule('generateNftsUI');
      if (generateNftsUI.updateTaskStatus) {
        generateNftsUI.updateTaskStatus('selecting-traits', 'completed');
    ...
    return Promise.all(imagePromises).then(() => {
      // Update task: Images loaded, start validating rules (in strict order)
      if (window.NFTApp && window.NFTApp.getModule && window.NFTApp.getModule('generateNftsUI')) {
        const generateNftsUI = window.NFTApp.getModule('generateNftsUI');
        if (generateNftsUI.updateTaskStatus) {
          generateNftsUI.updateTaskStatus('loading-images', 'completed');
    ...
    if (!overrideRules && projectData.rules && projectData.rules.length > 0) {
      ...
      if (violations && violations.length > 0) {
        ...
        // No rules to validate - mark as completed
        if (window.NFTApp && window.NFTApp.getModule && window.NFTApp.getModule('generateNftsUI')) {
          const generateNftsUI = window.NFTApp.getModule('generateNftsUI');
          if (generateNftsUI.updateTaskStatus) {
            generateNftsUI.updateTaskStatus('validating-rules', 'completed');
      ...
      // Update task: Start composing canvas
      if (window.NFTApp && window.NFTApp.getModule && window.NFTApp.getModule('generateNftsUI')) {
        const generateNftsUI = window.NFTApp.getModule('generateNftsUI');
        if (generateNftsUI.updateTaskStatus) {
          generateNftsUI.updateTaskStatus('composing-canvas', 'in-progress');
