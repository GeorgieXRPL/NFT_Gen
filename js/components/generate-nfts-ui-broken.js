...
window.NFTApp.registerModule("generateNftsUI", {
  ...
  updateTraitInfoPanel: function(nft, projectData) {
    ...
    styleElement.textContent = `
      ...
      .nft-trait-edit-btn {
        position: absolute;
        top: 4px; /* Reduced from 14px to 4px */
        right: 4px; /* Reduced from 26px to 4px */
        background: var(--accent-primary, #6c5ce7);
        color: #fff;
        border: none;
        border-radius: 3px; /* Reduced from 8px to 3px */
        padding: 1px 3px; /* Reduced from 8px 16px to 1px 3px */
        font-size: 8px; /* Reduced from 16px to 8px */
        font-weight: 500;
        cursor: pointer;
        z-index: 100;
    ...
    infoList.innerHTML = '';
    // Add a title to the trait list (outside the scrollable container)
    let traitTitle = infoList.querySelector('.trait-info-title');
    if (!traitTitle) {
      traitTitle = document.createElement('h3');
      traitTitle.className = 'trait-info-title';
      traitTitle.textContent = 'NFT Traits';
      infoList.appendChild(traitTitle);
    }
    ...
    if (projectData && projectData.traits) {
      projectData.traits.forEach((layer, idx) => {
        ...
        if (traitName === 'none') {
          ...
          traitNameElem.textContent = traitName.length > 14 ? traitName.slice(0, 12) + '…' : traitName;
          if (traitName.length > 14) {
            traitNameElem.title = traitName;
          }
        }
        // Bottom line: layer name (grey, left), rarity (purple, right)
        ...
        layerNameElem.textContent = layerName.length > 14 ? layerName.slice(0, 12) + '…' : layerName;
        if (layerName.length > 14) {
        ...
        bottomLine.appendChild(rarityElem);
    ...
    if (traitInfoPanel && !traitInfoPanel.querySelector('.trait-info-title')) {
      const traitTitle = document.createElement('h3');
      traitTitle.className = 'trait-info-title';
      traitTitle.textContent = 'NFT Traits';
      traitTitle.style.position = 'sticky';
      traitTitle.style.top = '0';
      traitTitle.style.zIndex = '2';
      traitTitle.style.background = '#23232b';
      traitTitle.style.margin = '0';
      traitTitle.style.padding = '16px 0 8px 0';
      traitTitle.style.textAlign = 'center';
      traitTitle.style.fontSize = '16px';
      traitTitle.style.fontWeight = '600';
      traitInfoPanel.insertBefore(traitTitle, traitInfoPanel.firstChild);
    }
    ...
    // Add dynamic height adjustment for traits panel
    window.setTraitsPanelHeight = function() {
      const batchPanel = document.querySelector('.batch-generation-panel');
      const nftGrid = document.getElementById('batch-nft-grid');
      const traitsPanel = document.querySelector('.traits-list-card');
      if (!batchPanel || !nftGrid || !traitsPanel) return;
      // Find the last NFT in the 10th row (index 29, or last NFT if fewer)
      const nftCards = nftGrid.querySelectorAll('.batch-nft-card');
      let bottom;
      if (nftCards.length >= 30) {
        bottom = nftCards[29].getBoundingClientRect().bottom;
      } else if (nftCards.length > 0) {
        bottom = nftCards[nftCards.length - 1].getBoundingClientRect().bottom;
      } else {
        // Fallback: just use the bottom of the batch panel
        bottom = batchPanel.getBoundingClientRect().bottom;
      }
      const top = batchPanel.getBoundingClientRect().top;
      const height = Math.max(0, bottom - top);
      traitsPanel.style.height = height + 'px';
      traitsPanel.style.minHeight = height + 'px';
      traitsPanel.style.maxHeight = height + 'px';
      traitsPanel.style.overflow = 'hidden';
      // Make the info panel and trait list fill the space
      const traitInfoPanel = traitsPanel.querySelector('.nft-trait-info-panel');
      if (traitInfoPanel) {
        traitInfoPanel.style.height = '100%';
        traitInfoPanel.style.maxHeight = '100%';
        traitInfoPanel.style.overflow = 'hidden';
      }
      const infoList = traitsPanel.querySelector('.nft-trait-info-list');
      if (infoList) {
        infoList.style.height = '100%';
        infoList.style.maxHeight = '100%';
        infoList.style.overflow = 'hidden';
      }
      const traitItemsContainer = traitsPanel.querySelector('.trait-items-container');
      if (traitItemsContainer) {
        traitItemsContainer.style.height = 'calc(100% - 40px)';
        traitItemsContainer.style.maxHeight = 'calc(100% - 40px)';
        traitItemsContainer.style.overflowY = 'auto';
      }
    };
    window.addEventListener('resize', window.setTraitsPanelHeight);
    setTimeout(window.setTraitsPanelHeight, 200);
  },
  
  updateSinglePreviewPanel: function(projectData, nft = null) {
    // --- ENSURE PREVIEW AND TRAIT INFO ARE CLEARED BEFORE RENDERING ---
    ...
    // --- INJECT REPLACE SELECTED NFT BUTTON IF CONDITIONS MET ---
    setTimeout(() => {
      const previewPanel = document.querySelector('.nft-preview-image-area');
      const previewContainer = previewPanel && previewPanel.querySelector('#nft-preview-container');
      const previewItem = previewContainer && previewContainer.querySelector('.nft-preview-item');
      // Only show if batch panel exists and exactly 1 NFT is selected
      const batchPanel = window.BatchGenerationPanel && window.BatchGenerationPanel.instance;
      // HIDE the button if in edit mode
      if (
        previewPanel && previewItem &&
        batchPanel &&
        batchPanel.nftBatch && batchPanel.nftBatch.length > 0 &&
        batchPanel.selectedIndexes && batchPanel.selectedIndexes.size === 1 &&
        window.lastGeneratedNFT && window.lastGeneratedNFT.seed &&
        batchPanel.editingIndex === null
      ) {
        // Remove any previous button
        let oldBtn = previewPanel.querySelector('.replace-selected-nft-btn');
        if (oldBtn) oldBtn.remove();
        // Create button
        const btn = document.createElement('button');
        btn.className = 'replace-selected-nft-btn btn btn-primary';
        btn.textContent = 'Replace Selected NFT';
        ...
        btn.style.fontFamily = 'Archivo, Arial, sans-serif';
        btn.title = 'Replace the selected NFT in the batch with this preview NFT (must be unique)';
        // Tooltip (if app uses custom tooltips, add here)
        btn.setAttribute('data-tooltip', 'Replace the selected NFT in the batch with this preview NFT (must be unique)');
        // Click handler
        btn.onclick = function(e) {
          e.stopPropagation();
          // Get selected index
          const selectedIdx = Array.from(batchPanel.selectedIndexes)[0];
          // Check uniqueness (seed must not exist elsewhere in batch)
          const previewSeed = window.lastGeneratedNFT.seed;
          const existsElsewhere = batchPanel.nftBatch.some((nft, idx) => idx !== selectedIdx && nft.seed === previewSeed);
          if (existsElsewhere) {
            if (window.NFTApp.getModule('notificationService')) {
              window.NFTApp.getModule('notificationService').show('This NFT already exists in the batch. Each NFT must be unique.', 'warning', 2500);
            }
            return;
          }
          // Replace NFT in batch
          if (typeof batchPanel.replaceNftAtIndex === 'function') {
            batchPanel.replaceNftAtIndex(selectedIdx, window.lastGeneratedNFT);
          } else {
            // Fallback: direct replace
            batchPanel.nftBatch[selectedIdx] = JSON.parse(JSON.stringify(window.lastGeneratedNFT));
            batchPanel.renderBatchPanels();
          }
          if (window.NFTApp.getModule('notificationService')) {
            window.NFTApp.getModule('notificationService').show('NFT replaced successfully.', 'success', 2000);
          }
        };
        ...
        let oldBtn = previewPanel && previewPanel.querySelector('.replace-selected-nft-btn');
        if (oldBtn) oldBtn.remove();
      }
    }, 0);
  },
    ...
    try {
      ...
      try {
        nftPromise = window.NFTApp.getModule('generateNfts').generateSingleNFT(
          projectData, overrideRules, randomSeed
        );
        nftPromise.then(nft => {
        ...
        const rarityScore = nft.averageRarityScore !== undefined ? nft.averageRarityScore.toFixed(2) : 'N/A';
        console.log("Rarity tier:", nft.overallTier, `(${rarityScore}%)`);
        ...
        const wasSeedToggleEnabled = singleSeedToggle ? singleSeedToggle.checked : false;
  ...
  generateSingleNFTWithSeed: function(projectData) {
    ...
    const wasToggleEnabled = singleSeedToggle ? singleSeedToggle.checked : false;
    ...
    generatePromise.then(nft => {
      ...
      if (singleSeedToggle && singleSeedToggle.checked !== wasToggleEnabled) {
        singleSeedToggle.checked = wasToggleEnabled;
  ...
  displaySingleNFT: function(nft) {
    // --- ENSURE PREVIEW IS CLEARED BEFORE RENDERING ---
    const previewPanelElem = document.querySelector('.nft-preview-image-area');
    if (previewPanelElem) previewPanelElem.innerHTML = '';
    // --- END CLEAR ---
    if (nft) window.lastGeneratedNFT = nft;
    ...
    let previewContainer = previewPanelElem ? previewPanelElem.querySelector('#nft-preview-container') : null;
    if (previewPanelElem && !previewContainer) {
      previewContainer = document.createElement('div');
      previewContainer.id = 'nft-preview-container';
      previewPanelElem.appendChild(previewContainer);
    }
    if (!previewPanelElem || !previewContainer) {
      console.warn('[DEBUG] .nft-preview-image-area or #nft-preview-container not found in DOM');
      if (window.NFTApp) window.NFTApp.isRenderingNFT = false;
      return;
    }
    // Remove welcome and placeholder messages if present
    const welcomeElem = previewPanelElem.querySelector('.nft-preview-welcome');
    if (welcomeElem && welcomeElem.parentNode) {
  ...
  showTraitSelectionModal: function(layerName, traitIndex, nft, projectData) {
    ...
    // Modal HTML with loading state in body
      modal.innerHTML = `
        <div class="modal-dialog" style="max-width: 640px; min-width: 320px; background: #222; border-radius: 10px; box-shadow: 0 8px 32px rgba(0,0,0,0.4); margin: 40px auto; padding: 0; position: relative;">
          <button class="modal-close-btn" style="background: none; border: none; color: #aaa; font-size: 22px; cursor: pointer; position: absolute; top: 16px; right: 20px; z-index: 10;">&times;</button>
        <div class="modal-body" style="padding: 16px 20px; min-height: 180px; display: flex; flex-direction: column; align-items: center; justify-content: center;">
          <div class="trait-list-loading" style="display: flex; flex-direction: column; align-items: center; justify-content: center; width: 100%; height: 100%; min-height: 120px;">
            <div style="font-size: 16px; color: #fff; margin-bottom: 18px;">Traits list being populated</div>
            <div class="trait-list-spinner" style="display: flex; align-items: center; justify-content: center; height: 48px;">
              <div class="trait-spinner" style="width: 38px; height: 38px; border: 4px solid #6c5ce7; border-top: 4px solid #222; border-radius: 50%; animation: trait-spin 1s linear infinite;"></div>
            </div>
          </div>
          </div>
          <div class="modal-footer" style="display: flex; justify-content: flex-end; gap: 10px; padding: 12px 20px 16px 20px; border-top: 1px solid #333;">
            <button id="modal-cancel-btn" class="btn btn-secondary" style="min-width: 80px;">Cancel</button>
            <button id="modal-apply-btn" class="btn btn-primary" style="min-width: 80px;">Apply</button>
          </div>
        </div>
      `;
    ...
    setTimeout(() => {
      ...
      modalBody.innerHTML = `
        <div style="margin-bottom: 10px; position: relative; display: block;">
          <input type="text" class="trait-search-input" placeholder="Search traits..." style="width: 100%; padding: 6px 10px; border-radius: 4px; border: 1px solid #444; background: #181818; color: #fff; font-size: 14px;">
          <button class="trait-search-clear" style="background: none; border: none; color: #aaa; font-size: 16px; cursor: pointer; position: absolute; right: 8px; top: 50%; transform: translateY(-50%);">&times;</button>
        </div>
        <div class="trait-count-indicator" style="font-size: 12px; color: #aaa; margin-bottom: 8px;"></div>
        <div class="trait-selection-list"></div>
      `;
      // Now run the rest of the trait list population logic as before
      const traitSelectionList = modalBody.querySelector('.trait-selection-list');
      ...
      if (!layer) {
        ...
        noneNameDiv.innerHTML = `<span class="trait-name-ellipsis-select">none</span><span class="custom-tooltip">none</span>`;
        ...
        layer.traits.forEach(trait => {
          ...
          nameDiv.innerHTML = `<span class="trait-name-ellipsis-select">${trait.name}</span><span class="custom-tooltip">${trait.name}</span>`;
      ...
      function filterTraits() {
        ...
        if (countIndicator) {
          if (val === '') {
            countIndicator.textContent = `${traitThumbs.length - 1} traits`;
          } else {
      ...
      allThumbs.forEach(thumb => {
        ...
        thumb.style.zIndex = '';
        const indicator = thumb.querySelector('.trait-selected-indicator');
        ...
        if (thumb.getAttribute('data-trait-id') === currentTraitId) {
          ...
          checkmark.style.color = '#fff'; // Always white
          checkmark.style.background = '#4CAF50'; // Always green circle
        ...
        if (thumb.style.pointerEvents !== 'none') {
          ...
          allThumbs.forEach(t => {
            ...
            t.style.boxShadow = '';
            t.style.zIndex = '';
              const ind = t.querySelector('.trait-selected-indicator');
              if (ind) ind.remove();
          });
            ...
            if (!this.querySelector('.trait-selected-indicator')) {
            const checkmark = document.createElement('div');
            checkmark.className = 'trait-selected-indicator';
            checkmark.textContent = '✓';
            checkmark.style.position = 'absolute';
  ...
  applyTraitChange: function(modal, layerName, traitIndex, nft, projectData) {
    ...
    const doApply = () => {
      ...
      // Regenerate the NFT using the current trait selection for full determinism
      const generateNftsCore = window.NFTApp.getModule('generateNfts');
      if (generateNftsCore && typeof generateNftsCore.generateSingleNFT === 'function') {
        generateNftsCore.generateSingleNFT(projectData, true).then(newNft => {
          ...
          if (window.BatchGenerationPanel && window.BatchGenerationPanel.instance) {
            window.BatchGenerationPanel.instance.checkForTraitChanges();
          }
          if (window.NFTApp && window.NFTApp.getModule && window.NFTApp.getModule("notificationService")) {
            window.NFTApp.getModule("notificationService").show(
  ...
  setupUI: function(projectData) {
    ...
    generateNftsTab.innerHTML = `
      ...
      <p class="section-description">Create and preview NFT images for your collection. Keep on addind the NFTs you like to your seeds list (using 'Randomize' button for single generation or 'Generate Batch' for multiple Generations at once). Once you have the full collection seed list full you should proceed to the generate metadata tab. </p>
      <div class="generate-nfts-main-row" style="display: flex; flex-direction: row; gap: 12px; align-items: flex-start; width: 100%;">
        <div class="nft-preview-panel" style="flex: 0 0 440px; max-width: 440px; min-width: 340px; display: flex; flex-direction: column; gap: 0;">
          <div class="nft-controls-panel" style="display: flex; flex-direction: column; gap: 0; width: 100%; background: #23232b; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.10); padding: 18px 18px 10px 18px; margin-bottom: 18px; border: 1.5px solid #29293a;">
            <div class="nft-collection-seed-row"></div>
            <div class="nft-rarity-seed-row"></div>
            <div class="nft-seedlist-random-row"></div>
            <div class="nft-seedinput-row"></div>
          </div>
          <div class="nft-preview-image-area"><div id="nft-preview-container"></div></div>
          <div id="display-full-seed-list-btn-mount"></div>
          <div id="batch-generation-panel-mount" style="width: 100%; max-width: 100%; margin-top: 18px;"></div>
        </div>
        <div class="traits-list-card" style="flex: 1 1 0; min-width: 320px; max-width: 320px; display: flex; flex-direction: column;">
          <div class="nft-trait-info-panel">
            <ul class="nft-trait-info-list"></ul>
          </div>
        </div>
        <div id="batch-nft-grid-outer" style="flex: 1 1 0; min-width: 400px; max-width: 1000px; display: flex; flex-direction: column; margin-left: 12px;"></div>
      </div>
    `;
    // Now insert the button after the preview panel is present
    setTimeout(() => {
      const btnMount = generateNftsTab.querySelector('#display-full-seed-list-btn-mount');
      if (btnMount && !document.getElementById('display-full-seed-list-btn')) {
        let fullSeedListBtn = document.createElement('button');
        fullSeedListBtn.id = 'display-full-seed-list-btn';
        fullSeedListBtn.className = 'btn btn-primary';
        fullSeedListBtn.style.margin = '18px 0 0 0';
        fullSeedListBtn.style.width = '100%';
        fullSeedListBtn.style.fontSize = '15px';
        fullSeedListBtn.style.height = '38px';
        fullSeedListBtn.style.position = 'relative';
        fullSeedListBtn.innerHTML = 'Display full seed NFTs list';
        // Tooltip
        let tooltip = document.createElement('span');
        tooltip.className = 'tooltiptext';
        tooltip.textContent = 'Click to display the entire NFT collection on the Batch Generation so that it can be fine tuned';
        ...
        fullSeedListBtn.appendChild(tooltip);
        fullSeedListBtn.addEventListener('mouseenter', function() { tooltip.style.display = 'block'; });
        fullSeedListBtn.addEventListener('mouseleave', function() { tooltip.style.display = 'none'; });
        fullSeedListBtn.onclick = function() {
          if (window.displayFullSeedListNFTs) window.displayFullSeedListNFTs();
        };
        btnMount.appendChild(fullSeedListBtn);
        
        // Add rule violation buttons
        const ruleViolationContainer = document.createElement('div');
        ruleViolationContainer.style.display = 'flex';
        ruleViolationContainer.style.gap = '10px';
        ruleViolationContainer.style.marginTop = '10px';
        
        // List All seeds with Rule Violations button
        const listViolationsBtn = document.createElement('button');
        listViolationsBtn.id = 'list-rule-violations-btn';
        listViolationsBtn.className = 'btn btn-warning';
        listViolationsBtn.style.flex = '1';
        listViolationsBtn.style.fontSize = '14px';
        listViolationsBtn.style.height = '36px';
        listViolationsBtn.innerHTML = 'List All seeds with Rule Violations';
        listViolationsBtn.onclick = function() {
          console.log('List violations button clicked');
          console.log('window.listSeedsWithRuleViolations:', window.listSeedsWithRuleViolations);
          if (window.listSeedsWithRuleViolations) {
            window.listSeedsWithRuleViolations();
          } else {
            console.error('listSeedsWithRuleViolations function not found');
            alert('Function not available. Please refresh the page.');
          }
        };
        
        // Delete All seeds with Rule Violations button
        const deleteViolationsBtn = document.createElement('button');
        deleteViolationsBtn.id = 'delete-rule-violations-btn';
        deleteViolationsBtn.className = 'btn btn-danger';
        deleteViolationsBtn.style.flex = '1';
        deleteViolationsBtn.style.fontSize = '14px';
        deleteViolationsBtn.style.height = '36px';
        deleteViolationsBtn.innerHTML = 'Delete All seeds with Rule Violations';
        deleteViolationsBtn.onclick = function() {
          console.log('Delete violations button clicked');
          console.log('window.deleteSeedsWithRuleViolations:', window.deleteSeedsWithRuleViolations);
          if (window.deleteSeedsWithRuleViolations) {
            window.deleteSeedsWithRuleViolations();
          } else {
            console.error('deleteSeedsWithRuleViolations function not found');
            alert('Function not available. Please refresh the page.');
          }
        };
        
        ruleViolationContainer.appendChild(listViolationsBtn);
        ruleViolationContainer.appendChild(deleteViolationsBtn);
        btnMount.appendChild(ruleViolationContainer);
      }
    }, 0);
    // Mount Batch Generation Panel below the button
    setTimeout(function() {
      if (window.BatchGenerationPanel) {
        var mount = document.getElementById('batch-generation-panel-mount');
        if (mount) {
          mount.innerHTML = '';
          var batchPanel = new window.BatchGenerationPanel();
          batchPanel.container.style.width = '100%';
          batchPanel.container.style.maxWidth = '100%';
          batchPanel.container.style.minWidth = '0';
          mount.appendChild(batchPanel.getElement());
        }
      }
    }, 0);
    // Render NFT grid (batch results) in the right column using batch data
    function renderBatchNftGrid() {
      var gridOuter = document.getElementById('batch-nft-grid-outer');
      if (!gridOuter) return;
      gridOuter.innerHTML = '';
      var batchPanel = window.BatchGenerationPanel && window.BatchGenerationPanel.instance;
      if (!batchPanel || !Array.isArray(batchPanel.nftBatch)) return;
      // Create grid container with consistent 10-column layout
      var grid = document.createElement('div');
      grid.className = 'batch-nft-grid';
      grid.id = 'nft-grid'; // Use consistent ID
      grid.style.display = 'grid';
      grid.style.gridTemplateColumns = 'repeat(10, 118px)'; // Consistent 10 columns
      grid.style.gap = '6px'; // Consistent gap
      grid.style.justifyContent = 'flex-start';
      grid.style.justifyItems = 'center';
      grid.style.width = '1250px'; // Consistent width
      grid.style.minWidth = '1250px';
      grid.style.height = '1214px'; // Fixed height for consistent layout
      grid.style.overflowY = 'auto';
      grid.style.padding = '8px';
      
      // Set up the outer container for consistent layout
      gridOuter.style.overflowX = 'auto';
      gridOuter.style.overflowY = 'hidden';
      gridOuter.style.width = '100%';
      gridOuter.style.minWidth = '1300px';
      gridOuter.style.maxWidth = '100%';
      gridOuter.style.display = 'flex';
      gridOuter.style.flexDirection = 'column';
      gridOuter.style.boxSizing = 'border-box';
      // Render NFT cards
      batchPanel.nftBatch.forEach((nft, i) => {
        const card = batchPanel.createNftCard(nft, i);
        grid.appendChild(card);
      });
      gridOuter.appendChild(grid);
      // --- Pagination controls for batch NFT grid ---
let pagination = gridOuter.parentNode.querySelector('#batch-nft-pagination');
if (!pagination) {
  pagination = document.createElement('div');
  pagination.id = 'batch-nft-pagination';
  pagination.style.display = 'flex';
  pagination.style.justifyContent = 'center';
  pagination.style.alignItems = 'center';
  pagination.style.gap = '8px';
  pagination.style.marginTop = '10px';
  gridOuter.parentNode.appendChild(pagination);
}
const cardsPerPage = 9 * 7;
const totalPages = Math.ceil(batchPanel.nftBatch.length / cardsPerPage);
if (typeof window.batchNftCurrentPage !== 'number') window.batchNftCurrentPage = 0;
let currentPage = window.batchNftCurrentPage;
function scrollToPage(pageIdx) {
  const scrollX = pageIdx * (130 + 5) * 9; // 9 cards + 8 gaps
  gridOuter.scrollTo({ left: scrollX, behavior: 'smooth' });
  window.batchNftCurrentPage = pageIdx;
  currentPage = pageIdx;
  updatePagination();
}
function updatePagination() {
  ...
  firstBtn.onclick = () => scrollToPage(0);
  pagination.appendChild(firstBtn);
  // '<' button
  const prevBtn = document.createElement('button');
  prevBtn.textContent = '<';
  prevBtn.disabled = currentPage === 0;
  prevBtn.onclick = () => scrollToPage(Math.max(0, currentPage - 1));
  pagination.appendChild(prevBtn);
  // Page buttons (show up to 7, centered on current page)
  ...
  for (let i = startPage; i <= endPage; i++) {
    ...
    pageBtn.onclick = () => scrollToPage(i);
    if (i === currentPage) pageBtn.style.background = '#6c5ce7';
  ...
  nextBtn.onclick = () => scrollToPage(Math.min(totalPages - 1, currentPage + 1));
  pagination.appendChild(nextBtn);
  // '>>' button
  const lastBtn = document.createElement('button');
  lastBtn.textContent = '>>';
  lastBtn.disabled = currentPage === totalPages - 1;
  lastBtn.onclick = () => scrollToPage(totalPages - 1);
  pagination.appendChild(lastBtn);
}
updatePagination();
setTimeout(() => scrollToPage(currentPage), 0);
    }
    // Hook into batch panel updates
    setTimeout(function() {
      if (window.BatchGenerationPanel && window.BatchGenerationPanel.instance) {
        var batchPanel = window.BatchGenerationPanel.instance;
        // Patch renderBatchPanels to also update the grid
        const origRender = batchPanel.renderBatchPanels.bind(batchPanel);
        batchPanel.renderBatchPanels = function() {
          origRender();
          renderBatchNftGrid();
        };
        // Initial render
    ...
    if (generateNftsTab && !generateNftsTab.querySelector('.section-title')) {
      ...
      desc.textContent = 'Create and preview NFT images for your collection.';
      generateNftsTab.insertBefore(desc, generateNftsTab.firstChild);
      generateNftsTab.insertBefore(title, desc);
    }
    ...
    if (generateNftsTab) {
      ...
      generateNftsTab.innerHTML = `
        ...
        <p class="section-description">Create and preview NFT images for your collection.</p>
        <div class="generate-nfts-main-row" style="display: flex; flex-direction: row; gap: 12px; align-items: flex-start; width: 100%;">
          <div class="nft-preview-panel" style="flex: 0 0 440px; max-width: 440px; min-width: 340px; display: flex; flex-direction: column; gap: 0;">
            <div class="nft-controls-panel" style="display: flex; flex-direction: column; gap: 0; width: 100%; background: #23232b; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.10); padding: 18px 18px 10px 18px; margin-bottom: 18px; border: 1.5px solid #29293a;">
              <div class="nft-collection-seed-row"></div>
      ...
      `;
      // Load Batch Generation Panel CSS if not already loaded
      if (!document.getElementById('batch-generation-panel-css')) {
        var batchCss = document.createElement('link');
        batchCss.rel = 'stylesheet';
        batchCss.type = 'text/css';
        batchCss.id = 'batch-generation-panel-css';
        batchCss.href = 'css/batch-generation-panel.css';
        document.head.appendChild(batchCss);
      }
      // Mount Batch Generation Panel
      setTimeout(function() {
        if (window.BatchGenerationPanel) {
          var mount = document.getElementById('batch-generation-panel-mount');
          if (mount) {
            mount.innerHTML = ''; // Clear previous panels
            var batchPanel = new window.BatchGenerationPanel();
            mount.appendChild(batchPanel.getElement());
          }
        }
      }, 0);
      // Always use an object for projectData to ensure controls render
      this._renderControlRows(projectData || {});
    ...
    // --- Render the collection seed row controls ---
    const collectionSeedRow = document.querySelector('.nft-collection-seed-row');
    if (collectionSeedRow) {
      collectionSeedRow.innerHTML = '';
      const rowFlex = document.createElement('div');
      rowFlex.style.display = 'flex';
      rowFlex.style.alignItems = 'center';
      rowFlex.style.margin = '0'; // Remove bottom margin for perfect stacking
      rowFlex.style.width = '100%';
      rowFlex.style.gap = '0';
      rowFlex.style.justifyContent = 'flex-end';
      // (Removed Unique NFTs toggle)
      // Collection seed toggle
      const collectionSeedToggleLabel = document.createElement('label');
      collectionSeedToggleLabel.style.display = 'flex';
      collectionSeedToggleLabel.style.alignItems = 'center';
      collectionSeedToggleLabel.style.gap = '6px';
      // Toggle button (styled switch)
      ...
      toggleWrapper.style.marginRight = '4px';
      toggleWrapper.style.position = 'relative';
      // Make the label clickable for the toggle and the text
      const collectionSeedToggle = document.createElement('input');
      collectionSeedToggle.type = 'checkbox';
      collectionSeedToggle.id = 'random-seed-toggle';
      collectionSeedToggle.style.opacity = '0';
      collectionSeedToggle.style.position = 'absolute';
      collectionSeedToggle.style.left = '0';
      collectionSeedToggle.style.top = '0';
      collectionSeedToggle.style.width = '36px';
      collectionSeedToggle.style.height = '20px';
      collectionSeedToggle.style.margin = '0';
      collectionSeedToggle.style.cursor = 'pointer';
      const toggleSlider = document.createElement('span');
      toggleSlider.style.width = '36px';
      toggleSlider.style.height = '20px';
      toggleSlider.style.background = '#444';
      toggleSlider.style.borderRadius = '12px';
      toggleSlider.style.position = 'relative';
      toggleSlider.style.transition = 'background 0.2s';
      ...
      function updateToggleUI() {
        if (collectionSeedToggle.checked) {
          toggleSlider.style.background = '#6c5ce7';
      ...
      collectionSeedToggle.addEventListener('change', updateToggleUI);
      // Make the whole label clickable
      collectionSeedToggleLabel.style.cursor = 'pointer';
      // Initial state
      updateToggleUI();
      toggleWrapper.appendChild(collectionSeedToggle);
      toggleWrapper.appendChild(toggleSlider);
      collectionSeedToggleLabel.appendChild(toggleWrapper);
      const collectionSeedText = document.createElement('span');
      collectionSeedText.textContent = 'General Seed';
      collectionSeedText.className = 'seed-label';
      collectionSeedToggleLabel.appendChild(collectionSeedText);
      // Collection seed input
      const collectionSeedInput = document.createElement('input');
      collectionSeedInput.type = 'text';
      collectionSeedInput.id = 'random-seed';
      collectionSeedInput.placeholder = 'Seed for all NFTs';
      collectionSeedInput.style.flex = '1';
      collectionSeedInput.style.height = '28px';
      collectionSeedInput.style.fontSize = '13px';
      collectionSeedInput.style.marginLeft = '16px'; // Move 1 character to the right
      collectionSeedInput.style.borderRadius = '4px';
      collectionSeedInput.style.border = '1px solid #444';
      collectionSeedInput.style.background = '#181818';
      collectionSeedInput.style.color = '#fff';
      collectionSeedInput.style.textAlign = 'left';
      collectionSeedInput.disabled = true;
      // Order: toggle/label left, input right
      rowFlex.appendChild(collectionSeedToggleLabel);
      rowFlex.appendChild(collectionSeedInput);
      collectionSeedRow.appendChild(rowFlex);
      // Always enforce unique NFTs in projectData.settings
      if (projectData && projectData.settings) {
        projectData.settings.generateUniqueNfts = true;
      }
      // Restore state from projectData.settings if present
      const settings = (projectData && projectData.settings) ? projectData.settings : {};
      collectionSeedToggle.checked = !!settings.useRandomSeed;
      collectionSeedInput.value = '';
      collectionSeedInput.disabled = !collectionSeedToggle.checked;
      // Event listeners
      collectionSeedToggle.addEventListener('change', () => {
        collectionSeedInput.disabled = !collectionSeedToggle.checked;
        if (projectData && projectData.settings) {
          projectData.settings.useRandomSeed = collectionSeedToggle.checked;
          // When toggled off, clear the input field and projectData.settings.randomSeed
          if (!collectionSeedToggle.checked) {
            collectionSeedInput.value = '';
            projectData.settings.randomSeed = '';
          }
        }
      });
      collectionSeedInput.addEventListener('input', () => {
        if (projectData && projectData.settings) {
          if (collectionSeedToggle.checked) {
            projectData.settings.randomSeed = collectionSeedInput.value;
          }
        }
      });
    }
  ...
  showWelcomeMessageInPreviewPanel: function() {
    ...
    if (previewPanel) {
      previewPanel.innerHTML = `
        <div class="nft-preview-welcome" style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; min-height: 200px; padding: 20px; text-align: center; color: var(--text-secondary);">
          ...
          <div style=\"font-size: 18px; font-weight: 600; margin-bottom: 8px;\">Welcome to the NFT Generator!</div>
          <div style=\"font-size: 14px; line-height: 1.5;\">Start by generating an NFT to see its preview here.</div>
      </div>
    `;
    }
  },
  ...
  _renderControlRows: function(projectData) {
    ...
    if (seedListRandomRow) {
      ...
      seedListCounter.textContent = `${seedList.length || 0} / ${totalSupply}`;
      // Plus
      const plusBtn = document.createElement('button');
      plusBtn.id = 'seed-list-plus-btn';
      plusBtn.title = 'Add last NFT seed to list';
      ...
      plusBtn.innerHTML = '<span style="font-weight:bold;font-size:18px;">+</span>';
      plusBtn.addEventListener('click', function() {
        ...
        if (seedList.some(s => s.seed === lastNFT.seed)) {
          if (window.NFTApp && window.NFTApp.getModule && window.NFTApp.getModule('notificationService')) {
            window.NFTApp.getModule('notificationService').show('Seed already in the list.', 'warning', 2500);
          }
          return;
        }
        ...
        console.log('[DEBUG] plusBtn: seed added, counter updated:', seedList.length, totalSupply);
      });
      // Minus
      const minusBtn = document.createElement('button');
      minusBtn.id = 'seed-list-minus-btn';
      minusBtn.title = 'Manage saved seeds';
      ...
      minusBtn.innerHTML = '<span style="font-weight:bold;font-size:18px;">&minus;</span>';
      minusBtn.addEventListener('click', function() {
        // Use the new SavedSeedsModal class
        const modal = new window.SavedSeedsModal();
        modal.show(pd);
      });
        modal = document.createElement('div');
        modal.id = 'seed-list-modal';
        ...
        modal.innerHTML = `<div style="background:#222;padding:24px 16px 16px 16px;border-radius:10px;width:1400px !important;min-width:1400px !important;max-width:1400px !important;height:700px !important;min-height:700px !important;max-height:700px !important;overflow:hidden;display:flex;flex-direction:column;align-items:stretch;justify-content:flex-start;">
          <div style='display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;'>
            <span style='font-size:18px;font-weight:bold;color:#fff;'>Saved Seeds</span>
            <div style='display:flex;gap:8px;'>
              <button id='copy-seed-list-btn' style='background:#6c5ce7;color:#fff;border:none;border-radius:4px;padding:4px 10px;font-size:13px;cursor:pointer;'>Copy All</button>
              <button id='import-seed-list-btn' style='background:#27ae60;color:#fff;border:none;border-radius:4px;padding:4px 10px;font-size:13px;cursor:pointer;'>Import</button>
              <button id='close-seed-list-modal' style='background:none;border:none;color:#fff;font-size:22px;cursor:pointer;'>&times;</button>
            </div>
          </div>
          <div id='seed-list-modal-content' style='min-height:490px;height:490px;max-height:490px;overflow-y:auto;'></div>
          <div id='seed-list-pagination' style='display:flex;justify-content:center;align-items:center;margin-top:12px;gap:12px;'></div>
        </div>`;
        document.body.appendChild(modal);
        modal.style.display = 'flex';
        setTimeout(() => { modal.style.display = 'flex'; }, 50);
        console.log('[DEBUG] Modal created and appended to body');
        modal.querySelector('#close-seed-list-modal').onclick = () => modal.remove();
        function closeOnEscSeedList(e) {
          if (e.key === 'Escape') {
            if (document.getElementById('seed-list-modal')) {
              modal.remove();
              document.removeEventListener('keydown', closeOnEscSeedList);
            }
          }
        }
        document.addEventListener('keydown', closeOnEscSeedList);

        const pd = window.NFTApp.getModule('generateNftsUI') && window.NFTApp.getModule('generateNftsUI').projectData ? window.NFTApp.getModule('generateNftsUI').projectData : window.currentProject;
        const seedListKey = getSeedListKey(pd);
        let seedList = JSON.parse(localStorage.getItem(seedListKey) || '[]');
        console.log('[DEBUG] Seed list modal opened:', {
          projectData: pd,
        ...
        // Debug: Check all localStorage keys related to seeds
        console.log('[DEBUG] All localStorage keys:', Object.keys(localStorage).filter(key => key.includes('nftSeedList')));
        console.log('[DEBUG] Raw localStorage data for key:', localStorage.getItem(seedListKey));
        console.log('[DEBUG] Loaded seed list from localStorage:', seedList);
        const contentDiv = modal.querySelector('#seed-list-modal-content');
        const copyAllBtn = modal.querySelector('#copy-seed-list-btn');
        const importBtn = modal.querySelector('#import-seed-list-btn');
        const paginationDiv = modal.querySelector('#seed-list-pagination');
        ...
        if (!contentDiv) {
          console.log('[ERROR] contentDiv not found!');
        }
        let currentPage = 1;
        const pageSize = 30; // Always 30 per page (10×3 grid)

        // --- Caching for generated NFT images by seed (session only) ---
        const nftImageCache = {};

        async function getImageForSeed(seed) {
          console.log('[DEBUG] getImageForSeed called for seed:', seed);
          if (nftImageCache[seed]) {
            console.log('[DEBUG] Found cached image for seed:', seed);
            return nftImageCache[seed];
          }
          ...
          if (window.lastGeneratedNFT && window.lastGeneratedNFT.seed === seed && window.lastGeneratedNFT.imageData) {
            ...
            nftImageCache[seed] = window.lastGeneratedNFT.imageData;
            return window.lastGeneratedNFT.imageData;
          }
          // Try to generate NFT for this seed using the projectData
          if (window.NFTApp && window.NFTApp.getModule && window.NFTApp.getModule('generateNfts')) {
            try {
              console.log('[DEBUG] Generating NFT for seed:', seed);
              const nft = await window.NFTApp.getModule('generateNfts').generateSingleNFT(pd, false, seed);
              if (nft && nft.imageData) {
                console.log('[DEBUG] Successfully generated NFT image for seed:', seed);
                nftImageCache[seed] = nft.imageData;
                return nft.imageData;
              } else {
                console.log('[DEBUG] NFT generation failed for seed:', seed, 'nft:', nft);
              }
            } catch (e) {
              console.log('[DEBUG] Error generating NFT for seed:', seed, 'error:', e);
              // Ignore errors, fallback to null
            }
          } else {
            console.log('[DEBUG] generateNfts module not available');
          }
          console.log('[DEBUG] No image found for seed:', seed);
          return null;
        }

        // --- Modal for large NFT preview ---
        function showLargePreview(imageData) {
          let overlay = document.getElementById('nft-large-preview-overlay');
          if (overlay) overlay.remove();
          overlay = document.createElement('div');
          overlay.id = 'nft-large-preview-overlay';
          ...
          overlay.innerHTML = `<div style='background:#222;padding:24px;border-radius:12px;box-shadow:0 8px 32px rgba(0,0,0,0.5);max-width:90vw;max-height:90vh;display:flex;align-items:center;justify-content:center;'><img src='${imageData}' alt='NFT Preview' style='max-width:80vw;max-height:80vh;display:block;border-radius:8px;box-shadow:0 2px 16px rgba(0,0,0,0.4);'></div>`;
          document.body.appendChild(overlay);
          function closeOverlay() { overlay.remove(); document.removeEventListener('keydown', escHandler); }
        ...
        async function renderPage(page) {
          console.log('[DEBUG] renderPage called with page:', page, 'seedList.length:', seedList.length);
          console.log('[DEBUG] contentDiv exists:', !!contentDiv);
          console.log('[DEBUG] paginationDiv exists:', !!paginationDiv);
          contentDiv.innerHTML = '';
          if (!seedList.length) {
            console.log('[DEBUG] No seeds found, showing empty message');
            contentDiv.innerHTML = `<div style='color:#fff;text-align:center;margin-top:40px;'>No saved seeds.</div>`;
            paginationDiv.innerHTML = '';
            return;
          }
          const totalPages = Math.ceil(seedList.length / pageSize);
          // Clamp page
          if (page < 1) page = 1;
          if (page > totalPages) page = totalPages;
          currentPage = page;
          // Always show exactly 30 seeds per page (if available)
          let start = (page - 1) * pageSize;
          let end = Math.min(start + pageSize, seedList.length);
          const pageSeeds = seedList.slice(start, end);
          const list = document.createElement('div');
          list.style.display = 'grid';
          list.style.gridTemplateColumns = 'repeat(10, 118px)'; // Match batch grid exactly
          list.style.gap = '6px';
          list.style.padding = '8px 0';
          list.style.width = '1250px';
          list.style.height = '270px';
          list.style.overflow = 'visible';
          list.style.justifyContent = 'center';
          list.style.justifyItems = 'center';
          
          for (const seedObj of pageSeeds) {
            console.log('[DEBUG] Processing seed:', seedObj);
            
            // Create NFT card similar to batch generation panel
            const card = document.createElement('div');
            card.className = 'batch-nft-card';
            ...
            card.style.border = '1px solid #333'; // Normal border
            // card.style.boxShadow = '0 0 10px blue'; // Remove debug glow
            
            // Seed number display - full seed
            const seedDiv = document.createElement('div');
            seedDiv.className = 'seed-number';
            seedDiv.textContent = `Seed: ${seedObj.seed}`;
            seedDiv.style.fontSize = '7px';
            seedDiv.style.lineHeight = '1.2';
            seedDiv.style.marginBottom = '6px';
            seedDiv.style.width = '100%';
            seedDiv.style.height = '30px';
            seedDiv.style.textAlign = 'center';
            seedDiv.style.wordBreak = 'break-all';
            seedDiv.style.display = 'flex';
            seedDiv.style.alignItems = 'center';
            seedDiv.style.justifyContent = 'center';
            seedDiv.style.background = 'rgba(0,0,0,0.5)';
            seedDiv.style.borderRadius = '3px';
            seedDiv.style.padding = '2px';
            seedDiv.style.color = '#fff';
            // Thumbnail container
            const thumb = document.createElement('div');
            thumb.className = 'seed-list-thumb';
            thumb.style.position = 'relative';
            thumb.style.width = '100px';
            thumb.style.height = '100px';
            thumb.style.borderRadius = '4px';
            thumb.style.overflow = 'visible'; // Changed from 'hidden' to 'visible'
            thumb.style.background = '#2a2a2e';
            ...
            thumb.style.padding = '0'; // Removed padding that might interfere
            thumb.style.cursor = 'pointer';
            thumb.style.marginBottom = '8px';
            
            // Show loading state initially
            thumb.innerHTML = `<div>Loading...</div><div style="font-size:7px;margin-top:2px;">${seedObj.seed.substring(0, 6)}...</div>`;
            
            // Generate thumbnail asynchronously
            (async () => {
              try {
                console.log('[DEBUG] Generating thumbnail for seed:', seedObj.seed);
                const imageData = await getImageForSeed(seedObj.seed);
                if (imageData) {
                  console.log('[DEBUG] Successfully got image data for seed:', seedObj.seed);
                  const img = document.createElement('img');
                  img.src = imageData;
                  ...
                  console.log('[DEBUG] No image data for seed:', seedObj.seed);
                  thumb.innerHTML = `<div>No Image</div><div style="font-size:7px;margin-top:2px;">${seedObj.seed.substring(0, 6)}...</div>`;
                }
              } catch (error) {
                console.error('[DEBUG] Error generating thumbnail for seed:', seedObj.seed, error);
                thumb.innerHTML = `<div>Error</div><div style="font-size:7px;margin-top:2px;">${seedObj.seed.substring(0, 6)}...</div>`;
              }
            })();
            ...
            // Add three buttons: edit, copy, delete
            ...
            buttonContainer.style.background = 'rgba(0,255,0,0.5)'; // Debug green background
            buttonContainer.style.minHeight = '20px';
            buttonContainer.style.padding = '2px';
            buttonContainer.style.zIndex = '100';
            buttonContainer.style.border = '2px solid blue'; // Debug border
            ...
            editBtn.style.border = '1px solid white'; // Debug border
            ...
            copyBtn.style.border = '1px solid white'; // Debug border
            ...
            deleteBtn.style.border = '1px solid white'; // Debug border
            deleteBtn.addEventListener('click', async (e) => {
              e.stopPropagation();
              if (confirm(`Are you sure you want to delete seed ${seedObj.seed}?`)) {
                try {
                  // Remove from localStorage
                  let latestSeedList = JSON.parse(localStorage.getItem(seedListKey) || '[]');
                  const seedIndex = latestSeedList.findIndex(s => s.seed === seedObj.seed);
                  if (seedIndex !== -1) {
                    latestSeedList.splice(seedIndex, 1);
                    localStorage.setItem(seedListKey, JSON.stringify(latestSeedList));
                    
                    // Update the seedList variable
                    seedList = latestSeedList;
                    
                    // Update seed counter if it exists
                    const seedListCounter = document.getElementById('seed-list-counter');
                    if (seedListCounter) {
                      const totalSupply = getTotalSupply(pd);
                      seedListCounter.textContent = `${seedList.length} / ${totalSupply}`;
                    }
                    ...
                    // Show success message
                    if (window.NFTApp.getModule('notificationService')) {
                      window.NFTApp.getModule('notificationService').show('Seed deleted successfully', 'success', 2000);
                    }
                  }
                } catch (error) {
            ...
            console.log('[DEBUG] Card elements created:', {
              card: card,
              thumb: thumb,
              seedDiv: seedDiv,
          ...
          console.log('[DEBUG] Appending list to contentDiv, list children count:', list.children.length);
          contentDiv.appendChild(list);
          console.log('[DEBUG] List appended, contentDiv children count:', contentDiv.children.length);
          // Pagination
          paginationDiv.innerHTML = '';
          if (totalPages > 1) {
            // --- Show only 7 page buttons at a time ---
            let pageWindowStart = Math.max(1, currentPage - 3);
            let pageWindowEnd = Math.min(totalPages, pageWindowStart + 6);
            if (pageWindowEnd - pageWindowStart < 6) {
              pageWindowStart = Math.max(1, pageWindowEnd - 6);
            }
            // << button
            const firstBtn = document.createElement('button');
            firstBtn.textContent = '<<';
            firstBtn.style.background = '#333';
            ...
            // Page number buttons (max 7)
            for (let i = pageWindowStart; i <= pageWindowEnd; i++) {
              const pageBtn = document.createElement('button');
              pageBtn.textContent = i;
              pageBtn.style.background = i === currentPage ? '#6c5ce7' : '#333';
        ...
        // --- Show last page by default when modal opens ---
        const totalPagesInit = Math.ceil(seedList.length / pageSize);
        console.log('[DEBUG] About to call renderPage, totalPagesInit:', totalPagesInit, 'seedList.length:', seedList.length);
        
        // Force renderPage to be called
        setTimeout(() => {
          console.log('[DEBUG] Calling renderPage after timeout');
          renderPage(1);
        }, 100);
        
        if (totalPagesInit > 0) {
          currentPage = totalPagesInit;
          renderPage(currentPage);
        } else {
          renderPage(1);
        }
        
        // Test element removed - debugging is now in place
        // Remove any duplicate call to renderPage(currentPage) after this block.
        // After deleting or importing seeds, clamp currentPage to totalPages before calling renderPage
        ...
        if (copyAllBtn) {
          copyAllBtn.onclick = function() {
            let latestSeedList = JSON.parse(localStorage.getItem(seedListKey) || '[]');
            const seeds = latestSeedList.map(s => s.seed).join('\n');
        ...
        if (importBtn) {
          importBtn.onclick = function() {
            const input = prompt('Paste or enter seed numbers (one per line):');
            if (!input) return;
            ...
            for (const seed of newSeeds) {
              if (latestSeedList.some(s => s.seed === seed)) {
                skippedDuplicates++;
                continue;
              }
              if (latestSeedList.length + added >= totalSupply) {
                skippedSupply++;
                continue;
              }
              latestSeedList.push({ seed });
              added++;
            }
            if (added > 0) {
              localStorage.setItem(seedListKey, JSON.stringify(latestSeedList));
              ...
              seedListCounter.textContent = `${seedList.length || 0} / ${totalSupply}`;
              setSeedListCounterColor(seedListCounter, seedList.length || 0, totalSupply);
              let msg = `Imported ${added} new seed${added === 1 ? '' : 's'}.`;
              if (skippedDuplicates > 0) msg += ` Skipped ${skippedDuplicates} duplicate${skippedDuplicates === 1 ? '' : 's'}.`;
              if (skippedSupply > 0) msg += ` Skipped ${skippedSupply} due to supply limit.`;
              if (window.NFTApp.getModule('notificationService')) {
                window.NFTApp.getModule('notificationService').show(
                  msg,
                  'success',
              ...
              let msg = 'No new seeds were added.';
              if (skippedDuplicates > 0) msg += ` Skipped ${skippedDuplicates} duplicate${skippedDuplicates === 1 ? '' : 's'}.`;
              if (skippedSupply > 0) msg += ` Skipped ${skippedSupply} due to supply limit.`;
              if (window.NFTApp.getModule('notificationService')) {
                window.NFTApp.getModule('notificationService').show(
      ...
      leftGroup.appendChild(minusBtn);
      ...
      randomizeBtn.id = 'randomize-nft-btn';
      randomizeBtn.className = 'btn btn-primary';
      randomizeBtn.style.minWidth = '115px';
      randomizeBtn.style.width = '115px';
      randomizeBtn.style.height = '32px';
      randomizeBtn.style.fontSize = '14px';
      randomizeBtn.style.marginRight = '0';
      randomizeBtn.style.display = 'flex';
      randomizeBtn.style.alignItems = 'center';
      randomizeBtn.style.justifyContent = 'center';
      randomizeBtn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width: 16px; height: 16px; margin-right: 6px;"><path d="M21 2v6h-6"></path><path d="M3 12a9 9 0 0 1 15-6.7L21 8"></path><path d="M3 22v-6h6"></path><path d="M21 12a9 9 0 0 1-15 6.7L3 16"></path></svg>Randomize';
      ...
      rightGroup.appendChild(randomizeBtn);
    ...
    if (seedInputRow) {
      ...
      const singleSeedToggle = document.createElement('input');
      singleSeedToggle.type = 'checkbox';
      singleSeedToggle.id = 'single-seed-toggle';
      singleSeedToggle.style.margin = '0 4px 0 0';
      const slider = document.createElement('span');
      slider.className = 'toggle-slider';
      slider.style.marginRight = '4px';
      toggleLabel.appendChild(singleSeedToggle);
      toggleLabel.appendChild(slider);
      const labelText = document.createElement('span');
      labelText.className = 'toggle-text';
      labelText.textContent = 'Seed NFT';
      toggleLabel.appendChild(labelText);
      toggleRow.appendChild(toggleLabel);
      columnContainer.appendChild(toggleRow);

      // Second row: input and button
      const inputRow = document.createElement('div');
      inputRow.style.display = 'flex';
      inputRow.style.alignItems = 'center';
      inputRow.style.width = '100%';
      inputRow.style.gap = '0'; // Remove gap to control alignment
      ...
      seedInput.style.height = '28px';
      seedInput.style.fontSize = '13px';
      seedInput.style.margin = '0'; // Remove margin to avoid offset
      ...
      inputContainer.appendChild(seedInput);
      ...
      buttonContainer.style.minWidth = '115px';
      buttonContainer.style.marginRight = '14px'; // Move Generate Seed button 14px left
      ...
      generateSeedBtn.style.minWidth = '115px';
      generateSeedBtn.style.width = '115px';
      generateSeedBtn.style.height = '28px';
      generateSeedBtn.style.fontSize = '13px';
      generateSeedBtn.disabled = true;
      buttonContainer.appendChild(generateSeedBtn);
      inputRow.appendChild(buttonContainer);
      columnContainer.appendChild(inputRow);

      seedInputRow.appendChild(columnContainer);
      singleSeedToggle.addEventListener('change', function() {
        seedInput.disabled = !this.checked;
        generateSeedBtn.disabled = !this.checked;
      });
      seedInput.disabled = !singleSeedToggle.checked;
      generateSeedBtn.disabled = !singleSeedToggle.checked;
...
function disableNftGenButtons() {
  ...
  const randBtn = document.getElementById('randomize-nft-btn');
...
// Add a method to refresh the seed-list-counter
if (!window.NFTApp.getModule('generateNftsUI').refreshSeedListCounter) {
  window.NFTApp.getModule('generateNftsUI').refreshSeedListCounter = function() {
    const seedListCounter = document.getElementById('seed-list-counter');
    if (seedListCounter) {
      function getTotalSupply(pd) {
        ...
        if (totalSupplyInput && totalSupplyInput.value) {
          return parseInt(totalSupplyInput.value, 10) || 1;
        }
        if (pd && typeof pd.totalSupply !== 'undefined' && pd.totalSupply !== null && pd.totalSupply !== '') {
          return parseInt(pd.totalSupply, 10);
        } else if (pd && typeof pd.size !== 'undefined' && pd.size !== null && pd.size !== '') {
      ...
      seedListCounter.textContent = `${seedList.length || 0} / ${totalSupply}`;
      setSeedListCounterColor(seedListCounter, seedList.length || 0, totalSupply);
      console.log('[DEBUG] refreshSeedListCounter called:', seedList.length, totalSupply);
    }
  }
}
